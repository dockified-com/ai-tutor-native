...
export const generateCourse = task({
  ...
  run: async ({ courseId }: { courseId: string }) => {
    ...
    try {
      ...
      if (!pdfText.trim()) { await fail("OCR not supported in V1"); return; }
      ...
      const outline = JSON.parse(await aiRun("outline", pdfText.slice(0, 100000))) as {
        title: string;
        description: string;
      ...
      for (const l of outline.lessons) {
        const rows = await prisma.$queryRawUnsafe<{ id: string }[]>(
          `INSERT INTO lessons (course_id, position, title, summary, objectives) VALUES ($1::uuid,$2,$3,$4,$5) RETURNING id`,
      ...
      for (let i = 0; i < outline.lessons.length; i += CONCURRENCY) {
        ...
        const results = await Promise.allSettled(batch.map(async (l, bIdx) => {
          ...
          const blocksData = JSON.parse(await aiRun("generate-blocks", msg)) as {
          ...
          for (let p = 0; p < blocksData.blocks.length; p++) {
            ...
            await prisma.$executeRawUnsafe(
              `INSERT INTO blocks (lesson_id, position, type, content) VALUES ($1::uuid,$2,$3,$4::jsonb)`,
